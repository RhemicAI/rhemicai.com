...
const ENGINE_STAGES = [
  { label: 'Crawling site...', duration: 2000 },
  { label: 'Parsing schema markup...', duration: 1500 },
  { label: 'Analyzing content depth...', duration: 1800 },
  { label: 'Scoring AEO readiness...', duration: 1600 },
  { label: 'Generating recommendations...', duration: 1400 },
...
function InputColumn({ active, reduced }: { active: boolean; reduced: boolean }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="text-[10px] tracking-[0.2em] uppercase text-[var(--text-faint)] mb-3 font-mono">
        Website Data
      </div>
      <div className="flex-1 border border-[var(--border-default)] rounded-lg bg-black/30 p-3 overflow-hidden font-mono text-[11px] leading-relaxed relative">
        {/* Fade overlay at top */}
        <div className="absolute top-0 left-0 right-0 h-6 bg-gradient-to-b from-black/50 to-transparent z-10 pointer-events-none rounded-t-lg" />
        <div className="space-y-1">
          {lines.map((line, i) => (
            <div
              key={`${i}-${line}`}
              className="text-emerald-400/70 whitespace-nowrap overflow-hidden"
              style={{
                opacity: reduced ? 1 : 0,
...
function EngineColumn({ active, reduced }: { active: boolean; reduced: boolean }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="text-[10px] tracking-[0.2em] uppercase text-[var(--text-faint)] mb-3 font-mono text-center">
        AEO Engine
      </div>
      <div className="flex-1 border border-[var(--border-strong)] rounded-lg bg-black/40 p-4 font-mono text-xs relative overflow-hidden">
        ...
        <div className="text-center mb-3">
          <span className={`text-sm font-bold tracking-wider ${isComplete ? 'text-emerald-400' : 'text-[var(--text-primary)]'}`}>
            RHEMIC AI
          </span>
        </div>
        ...
        <div className="mb-3 text-center">
          <span className={`text-[11px] ${isComplete ? 'text-emerald-400' : 'text-amber-400/80'}`}>
            {currentStage?.label}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          {METRICS.map((m) => (
            <div key={m.label} className="text-center py-1.5 bg-white/[0.03] rounded border border-[var(--border-subtle)]">
              <div className={`text-sm font-bold ${isComplete ? 'text-emerald-400' : 'text-[var(--text-primary)]'}`}>
                {m.value}
              </div>
              <div className="text-[9px] text-[var(--text-faint)] uppercase tracking-wider">{m.label}</div>
            </div>
          ))}
        </div>
...
function OutputColumn({ active, reduced }: { active: boolean; reduced: boolean }) {
  ...
  useEffect(() => {
    ...
    }, 3000 + OUTPUT_CARDS.length * 1200 + 2000);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="text-[10px] tracking-[0.2em] uppercase text-[var(--text-faint)] mb-3 font-mono text-right">
        AI Answers
      </div>
      <div className="flex-1 space-y-2 overflow-hidden">
        {OUTPUT_CARDS.map((card, i) => {
          ...
          return (
            <div
              key={card.source}
              className="border border-[var(--border-default)] rounded-lg bg-black/30 p-3 font-mono text-[11px] leading-relaxed transition-all duration-500"
              style={{
                opacity: visible ? 1 : 0,
            ...
            >
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-[var(--text-secondary)] font-medium text-[10px] uppercase tracking-wider">
                  {card.source}
                </span>
                <span className="text-emerald-400 text-[10px] ml-auto">✓</span>
              </div>
              <div className="text-[var(--text-tertiary)] line-clamp-2">
                {card.text}
              </div>
            </div>
          );
  ...
  return (
    ...
    >
      ...
      <div className="scroll-reveal max-w-5xl mx-auto text-center mb-16">
        <p className="text-[10px] md:text-xs font-medium tracking-[0.3em] uppercase text-[var(--text-faint)] mb-4 font-mono">
          The Engine
        </p>
        ...
        <p className="text-sm md:text-base text-[var(--text-muted)] max-w-xl mx-auto font-light leading-relaxed">
