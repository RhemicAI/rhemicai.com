...
function applicationFormData(roleSlug = sdrRole.slug) {
  ...
  formData.set("resume", new File(["%PDF-1.4 test"], "resume.pdf", { type: "application/pdf" }));
  return formData;
}
...
describe("careers application route scoring", () => {
  ...
  it("does not call the scoring agent for coming-soon role applications", async () => {
    ...
    const request = new Request("http://localhost/api/careers/applications", {
      method: "POST",
      body: applicationFormData("client-success-ops-coordinator"),
    });
